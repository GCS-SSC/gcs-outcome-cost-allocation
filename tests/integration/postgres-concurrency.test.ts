...
const seedDraftScenario = async (
  ...
  await sql`
    ...
    ) VALUES (${budgetLineId}, ${budgetYearId}, 100)
  `.execute(db)
...
describe('outcome allocation PostgreSQL concurrency', () => {
    ...
    const version = await managedMutation(observerDb, '4', async trx => {
      ...
      await trx
        ...
        .values({
          ...
          agreement_budget_fiscal_year_id: '24',
          outcome_id: '32',
  ...
  it('enforces allocation lifecycle, coordinate, and provenance invariants in PostgreSQL', async () => {
    ...
    const allocation = await managedMutation(observerDb, '5', async trx => await trx
      ...
      .values({
        ...
        agreement_budget_fiscal_year_id: '26',
        outcome_id: '33',
        allocation_method: 'amount',
        allocation_value: 100
      })
    ...
    await expect(managedMutation(observerDb, '5', async trx => await trx
      ...
      .values({
        ...
        agreement_budget_fiscal_year_id: '28',
        outcome_id: '34',
        allocation_method: 'amount',
        allocation_value: 0
      })
  ...
  it('re-reads a denied status after waiting for generated payment creation and rechecks committed coverage', async () => {
    ...
    await managedMutation(observerDb, '12', async trx => {
      ...
      await trx
        ...
        .values({
          ...
          agreement_budget_fiscal_year_id: '35',
          outcome_id: '38',
          allocation_method: 'amount',
          allocation_value: 100,
          resolved_amount: 100,
    ...
    try {
      ...
      resurrecting = waiterDb.transaction().execute(async trx => {
        ...
        await trx
          .updateTable('Funding_Case_Agreement_Payment')
          .set({ egcs_fc_status: 'pendingapproval' })
          .where('id', '=', '190')
          .execute()
      })
      await vi.waitFor(async () => {
    ...
    const statuses = await observerDb
      ...
      .orderBy('id', 'asc')
      .execute()
    expect(statuses.map(payment => payment.egcs_fc_status)).toEqual(['denied', 'inprogress'])
  })
  ...
  it('snapshots commitment descriptions longer than five hundred characters', async () => {
    ...
    try {
      ...
      expect(commitmentLabel).toBe(`GL 5000 - ${description}`)
    ...
    const version = await managedMutation(observerDb, '2', async trx => {
      ...
      await trx
        ...
        .values({
          ...
          agreement_budget_fiscal_year_id: '20',
          outcome_id: '30',
          allocation_method: 'amount',
          allocation_value: 100
        })
    ...
    try {
      ...
      await expect(getSavedAllocations(
        ...
        expect.objectContaining({
          ...
          commitmentLabelEn: 'GL 5000 - Program',
          commitmentLabelFr: 'GL 5000 - Program',
    ...
    const version = await managedMutation(observerDb, '10', async trx => {
      ...
      await trx
        ...
        .values({
          ...
          agreement_budget_fiscal_year_id: '32',
          outcome_id: '36',
          allocation_method: 'amount',
          allocation_value: 100
        })
        ...
        .values([
          {
            ...
            agreement_budget_fiscal_year_id: '34',
            outcome_id: '37',
            allocation_method: 'amount',
            allocation_value: 50,
            resolved_amount: 50,
    ...
    const version = await managedMutation(observerDb, '3', async trx => {
      ...
      await trx
        ...
        .values({
          ...
          agreement_budget_fiscal_year_id: '22',
          outcome_id: '31',
          allocation_method: 'amount',
          allocation_value: 100
        })
